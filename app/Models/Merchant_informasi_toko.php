...
class Merchant_informasi_toko extends Model
{
    use HasFactory;
	private $_db,
    $_data;
	    protected $table = 'merchant_informasi_toko';
...
public function __construct($user = null) 
{
	date_default_timezone_set("Asia/Jakarta");
		...
		{
			$mit_login_id = $request->post('login_id');
			$data = DB::table("merchant_informasi_toko")->where('mit_login_id',$mit_login_id)->first();
			return $data;
		}
		public function informasitoko1(Request $request,$login_id,$phone_number){

        $mit_store_name = $request->post('mit_store_name');
        $mit_business_type = $request->post('mit_business_type');
        $mit_omzet_annually = $request->post('mit_omzet_annually');
        $mit_omzet_monthly = $request->post('mit_omzet_monthly');
        $mit_business_type = $request->post('mit_business_type');
        $mit_business_type_info = $request->post('mit_business_type_info');
		//$picture="";
		//$namafiledidb = $request->post('namafiledidb');
		
        //$featured_photo = $picture;
			$data = DB::table("merchant_informasi_toko")->where('mit_login_id',$login_id)->first();
			if(!$data)
			{
				$data = array(
					'mit_store_name' => $mit_store_name,
					'mit_business_type' => $mit_business_type,
					'mit_business_type_info' => $mit_business_type_info,
					'mit_omzet_annually' => $mit_omzet_annually,
					'mit_omzet_monthly'=>$mit_omzet_monthly,
					'mit_login_id' => $login_id,
					'created_by' => $phone_number,
					'created_date'=>date("Y-m-d H:i:s"),
				);
			//$inserted = DB::getInstance();
					$inserted=DB::table('merchant_informasi_toko')->insertOrIgnore($data);
			}
			else
			{
				$data = array(
				...
				$inserted=DB::table('merchant_informasi_toko')->where('mit_login_id',$login_id)->update($data);
			}
			if(isset($error))
			{
				return false;
